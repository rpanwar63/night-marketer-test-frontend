...
function UpdateUser({close, user, users, setUsers, index}) {
  ...
  return (
    <div className={`fixed inset-0 bg-black/70 flex items-center justify-center`} onClick={() => close(false)}>
      <div className={`flex flex-col px-10 py-5 bg-white shadow-lg rounded-md`} onClick={e => e.stopPropagation()}>
        ...
        <button className={`border border-green-600 rounded-md px-3 py-1 lg:px-5 lg:py-3 text-green-600`} onClick={addUser}>Update User</button>
    </div>
    </div>
  )
}
